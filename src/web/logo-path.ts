// Herdr 官方 logo 的共享来源（design: herdr-mode-gating §4.3）。
// - HerdrLogo 组件（pane-list.tsx）引用 HERDR_LOGO_PATH_D 渲染 <svg>；
// - Tab logo（styles.ts）用 herdrLogoMaskUrl() 构建 CSS mask data URI。
// 单一来源，避免两处维护 path 数据。

/** Herdr logo 的 path 数据（<g fill="currentColor" transform="translate(0 512) scale(.1 -.1)">）。 */
export const HERDR_LOGO_PATH_D =
...
/** 完整 <svg> 标记（fill 用 currentColor，随主题取色；CSS mask data URI 源）。 */
export const HERDR_LOGO_SVG =
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">' +
  '<g fill="currentColor" transform="translate(0 512) scale(.1 -.1)" stroke="none">' +
  '<path d="' + HERDR_LOGO_PATH_D + '"/>' +
  '</g></svg>'

/** CSS mask 用的 data URI（encodeURIComponent 保证引号/尖括号安全进入 url("...")）。 */
export function herdrLogoMaskUrl(): string {
  return 'data:image/svg+xml;utf8,' + encodeURIComponent(HERDR_LOGO_SVG)
}
